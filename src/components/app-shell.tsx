...
export function AppShell({children}:{children:React.ReactNode}){
  ...
  const {data,loading,dataError,refresh,busy,run,stop,workspace}=useCrm();
  ...
  function submit(e:FormEvent<HTMLFormElement>){
    e.preventDefault();
    if(prompt.trim()&&!busy){
      void run(prompt,workspace?.items.map(i=>i.deal.id).slice(0,4));
      setPrompt('');
    }
  }
  return <div className="app-shell">
    ...
    <div className="app-content">
      ...
      <section className="command-surface" aria-label="Ask your CRM">
